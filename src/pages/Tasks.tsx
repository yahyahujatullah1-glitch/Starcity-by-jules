...
export default function Tasks() {
  ...
  const handleAdd = async (e: any) => {
    ...
    await supabase.from('tasks').insert([{ title: formData.get('title'), due_date: formData.get('date'), status: 'Todo', assigned_to: user?.id }]);
    setIsOpen(false);
    refresh();
  };
  ...
  const handleSubmitProof = async (e: any) => {
    ...
    if(detailTask && proofLink) {
        ...
        setProofLink("");
        setDetailTask(null); // Close to refresh
        refresh();
    }
  };
        ...
        <Modal title="Task Details" onClose={() => setDetailTask(null)}>
          <div className="space-y-6">
            ...
            <div className="border-t border-border pt-4">
              ...
              {/* If proof exists, show it and admin controls */}
              {detailTask.proof_url ? (
                <div className="bg-white/5 p-4 rounded-lg border border-white/10 space-y-3">
                    <div className="flex items-center gap-2 text-blue-400 underline">
                        <ExternalLink size={16}/> <a href={detailTask.proof_url} target="_blank">{detailTask.proof_url}</a>
                    </div>
